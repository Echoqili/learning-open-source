...
本 Skill 依赖项目根目录的 `CLAUDE.md` 作为**项目级规则书**，该文件由 AI 启动时自动加载，包含：

- **硬约束**：编码阶段不得修改 spec 文档、不得实现未声明的功能、TDD 强制等 6 条红线
- **反模式速查**：7 个常见错误的正确做法对照表
- **沉淀教训**：策略反转原子提交、引用与决策点同位置、PowerShell 5.1 兼容性、git ignore 三合一检查
- **编码规范**：中日文注释允许、secrets 禁止提交、CLAUDE.md 只能追加不删旧

> 维护者须知：每次新增跨文件规则或沉淀新教训时，同步更新 `CLAUDE.md` 和本 Skill。CLAUDE.md 是 AI 的「第一本手册」，本 Skill 是「流水线操作手册」，二者互补。
